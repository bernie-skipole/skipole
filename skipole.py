...
# The following line assumes, as default, that projectfiles are in the same directory
# as this skipole.py file
...
# in skiboot.py, by calling skipoles.get_projectfiles(), which will return a path
# if it has been set into that dictionary by some other method
...
# however if it is imported as a module, then this function is available to
# create a wsgi application function for a previously built project

def create_application(project, options={}, projectfiles=None):
    "Returns the wsgi application, if projectfiles is None, uses the default given in PROJECTFILES"
    global PROJECTFILES
    if projectfiles:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--skiadmin", action='store_true', dest="skiadmin", default=False,
                      help="Load the %s project to administer the given project. With your browser call project_url/%s" % (adminproj,adminproj))
