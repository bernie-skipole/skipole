...
def submit_data(caller_ident, ident_list, submit_list, submit_dict, call_data, page_data, lang):
    ...
    # submit_list[0] is the string 'editcss' and has already been used to call this fuction
    assert submit_list[0] == 'editcss'

    if submit_list[1] == 'editcss':
        try:
            submitfunc = getattr(editcss, submit_list[2])
        except:
            raise FailPage("submit_list contains 'editcss', 'editcss', but the required function is not recognised")
