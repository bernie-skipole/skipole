...
def submit_data(caller_ident, ident_list, submit_list, submit_dict, call_data, page_data, lang):
    ...
    # submit_list[0] is the string 'editpages' and has already been used to call this fuction
    assert submit_list[0] == 'editpages'

    if submit_list[1] == 'common':
        try:
            submitfunc = getattr(common, submit_list[2])
        except:
            raise FailPage("submit_list contains 'editpages', 'common', but the required function is not recognised")
        return submitfunc(caller_ident, ident_list, submit_list, submit_dict, call_data, page_data, lang)
    elif submit_list[1] == 'editpage':
        try:
            submitfunc = getattr(editpage, submit_list[2])
        except:
