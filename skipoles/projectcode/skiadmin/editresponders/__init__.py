...
def submit_data(caller_ident, ident_list, submit_list, submit_dict, call_data, page_data, lang):
    ...
    # submit_list[0] is the string 'editresponders' and has already been used to call this fuction
    assert submit_list[0] == 'editresponders'

    if submit_list[1] == 'editrespondpage':
        try:
            submitfunc = getattr(editrespondpage, submit_list[2])
        except:
            raise FailPage("submit_list contains 'editresponders', 'editrespondpage', but the required function is not recognised")
