...
def submit_data(caller_ident, ident_list, submit_list, submit_dict, call_data, page_data, lang):
    ...
    # submit_list[0] is the string 'editsectionplaces' and has already been used to call this fuction
    assert submit_list[0] == 'editsectionplaces'

    if submit_list[1] == 'editplaceholder':
        try:
            submitfunc = getattr(editplaceholder, submit_list[2])
        except:
            raise FailPage("submit_list contains 'editsectionplaces', 'editplaceholder', but the required function is not recognised")
