...
def submit_data(caller_ident, ident_list, submit_list, submit_dict, call_data, page_data, lang):
    ...
    # submit_list[0] is the string 'editsections' and has already been used to call this fuction
    assert submit_list[0] == 'editsections'

    if submit_list[1] == 'managesections':
        try:
            submitfunc = getattr(managesections, submit_list[2])
        except:
            raise FailPage("submit_list contains 'editsections', 'managesections', but the required function is not recognised")
