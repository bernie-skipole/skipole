...
def submit_data(caller_ident, ident_list, submit_list, submit_dict, call_data, page_data, lang):
    ...
    # submit_list[0] is the string 'editvalidators' and has already been used to call this fuction
    assert submit_list[0] == 'editvalidators'

    if submit_list[1] == 'editvalidator':
        try:
            submitfunc = getattr(editvalidator, submit_list[2])
        except:
            raise FailPage("submit_list contains 'editvalidators', 'editvalidator', but the required function is not recognised")
