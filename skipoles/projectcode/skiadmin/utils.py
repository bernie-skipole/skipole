...
def domtree(project, pagenumber, section_name, location, contents, part_string_list, rows=1, indent=1):
    "Creates the contents of the domtable"

    part_loc = location[0]
    # if in a container
    if location[1]:
        part_loc = part_loc + '-' + str(location[1])

    partdict = fromjson.part_to_OD(project, pagenumber, section_name, location)
    if 'parts' not in partdict:
        return rows

    return _domparts(partdict, part_loc, contents, part_string_list, rows, indent)



def _domparts(partdict, part_loc, contents, part_string_list, rows, indent):
    ...
    for index, part in enumerate(parts):
        ...
        if part_type == 'Part':
            if last_row_at_this_level and (part_dict['tag_name'] != 'script') and (part_dict['tag_name'] != 'pre'):
                ...
                contents[last_row_at_this_level *9-4] = ['&searr;', 'width : 1%;', True, loc_string]
            last_row_at_this_level = rows
            rows = _domparts(part_dict, part_location_string, contents, part_string_list, rows, indent)
            # set u_r_flag for next item below this one
