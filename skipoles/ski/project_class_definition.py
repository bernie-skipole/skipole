...
class Project(object):
    ...
    def __init__(self, proj_ident, url="/"):
        ...
        # dictionary of textblocks with keys of (reference,language) and values being the text
        self.textblocks = {}
        # dictionary of textrefs with keys of reference and values being a list of languages for that textref
        self.textrefs = {}
        # set of available languages provided with textblocks
        self.languages = set()
    ...
    def textblocks_from_json(self):
        "Loads self.textblocks with the contents of the directory of json files"
        # dictionary of textblocks with keys of (reference,language) and values being the text
        self.textblocks = {}
        # dictionary of textrefs with keys of reference and values being a list of languages for that textref
        self.textrefs = {}
        ...
        self.languages = set()
        directory = skiboot.textblocks_json_directory(self._proj_ident)
        ...
        for filename in dir_contents:
            ...
            with open(filepath, 'r') as fp:
                ...
                for textref, text in textblocks_dict.items():
                   self.textblocks[(textref,language)] = text
                   if textref in self.textrefs:
                       if language not in self.textrefs[textref]:
                           self.textrefs[textref].append(language)
                   else:
                        self.textrefs[textref] = [language]
        # sort language lists
        for langlist in self.textrefs.values():
            langlist.sort()


    def textblocks_get_textrefs(self):
        "Returns a list of the textrefs"
        return [textref for textref in self.textrefs]

    def textblocks_textref_exists(self, textref):
        "Return True if the textref exists"
        return textref in self.textrefs
    ...
    def textblocks_get_text(self, textref, lang):
        ...
        if textref not in self.textrefs:
            return
        language, default_language = lang
        language = language.lower()
        # try preferred language
        if (textref,language) in self.textblocks:
            return self.textblocks[(textref,language)]
        # if not preferred for example en-gb, try en
        shortlang = language.split('-')
        if (textref,shortlang[0]) in self.textblocks:
            return self.textblocks[(textref,shortlang[0])]
        # try default language
        if (textref,default_language) in self.textblocks:
            return self.textblocks[(textref,default_language)]
        shortlang = default_language.split('-')
        if (textref,shortlang[0]) in self.textblocks:
            return self.textblocks[(textref,shortlang[0])]
        # try self.default_language
        if (textref,self.default_language) in self.textblocks:
            return self.textblocks[(textref,self.default_language)]
        shortlang = self.default_language.split('-')
        if (textref,shortlang[0]) in self.textblocks:
            return self.textblocks[(textref,shortlang[0])]
        # try skiboot_language
        skiboot_language = skiboot.default_language()
        if (textref,skiboot_language) in self.textblocks:
            return self.textblocks[(textref,skiboot_language)]
        shortlang = skiboot_language.split('-')
        if (textref,shortlang[0]) in self.textblocks:
            return self.textblocks[(textref,shortlang[0])]
        # try whatever language remains
        rlang = self.textrefs[textref][0]
        if (textref,rlang) in self.textblocks:
            return self.textblocks[(textref,rlang)]
    ...
    def textblocks_set_text(self, text, textref, language):
        ...
        language = language.lower()
        if not text:
            self.textblock_del_text(textref, language)
            return
        self.textblocks[textref, language] = text
        self.languages.update([language])
        if textref in self.textrefs:
            langlist = self.textrefs[textref]
        ...
        if shortlang[0] == language:
            return
        if (textref, shortlang) in self.textblocks:
            # A textblock with the given shortlang does exist, but check to see if it is just an empty string
            shortlangtext = self.textblocks[(textref, shortlang)]
            if (not shortlangtext) or (shortlangtext == " "):
                # add text
                self.textblock_set_text(text, textref, shortlang)
        else:
    ...
    def textblocks_del_text(self, textref, language):
        ...
        language = language.lower()
        if (textref,language) not in self.textblocks:
            return
        del self.textblocks[(textref,language)]
        langlist = self.textrefs[textref]
        try:
            pos = langlist.index(language)
            del langlist[pos]
            if not langlist:
                del self.textrefs[textref]
        except:
            pass
    ...
    def textblocks_del_textblock(self, textref):
        ...
        langlist = self.textrefs[textref].copy()
        for language in langlist:
            self.textblocks_del_text(textref, language)

    def textblocks_copy(self, sourceref, destinationref):
        "Copies textblock"
        langlist = self.textrefs[sourceref]
        self.textrefs[destinationref] = []
        destlanglist = self.textrefs[destinationref]
        for language in langlist:
            self.textblocks[(destinationref, language)] = self.textblocks[(sourceref, language)]
    ...
    def textblocks_to_json(self):
        ...
        for language in self.languages:
            filepath = os.path.join(directory, language + '.json')
            lang = (language, self.default_language)
            json_dict = {}
            for textref, langlist in self.textrefs.items():
                if language not in langlist:
                    continue
                json_dict[textref] = ''
                text = self.textblocks_get_text(textref, lang)
                if text is not None:
    ...
    def textblocks_get_reference_hierarchy(self):
        ...
        reference_list = [self.textrefs.keys()]
        return self._textblocks_make_hierarchy(reference_list)


    def _textblocks_make_hierarchy(self, reference_list):
        """Recursive function used by textblocks_get_reference_hierarchy
        ...
        for element, sublist in temp_dict.items():
            new_dict[element] = self._textblocks_make_hierarchy(sublist)
